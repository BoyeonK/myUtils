...
Workflow는 현재 repository를 조사하여 관련 구조와 자료를 발견하고, 발견한 프로젝트 고유 문맥을 현재 작업에 적극적으로 활용한다.

프로젝트 고유 구조가 현재 작업에서 확인되었다고 해서 다른 프로젝트에도 같은 구조가 존재한다고 가정하지 않는다.
...
이는 여러 프로젝트에서 Workflow를 일관되게 적용하고,
향후 자동화 또는 tooling이 동일한 위치를 기준으로 동작할 수 있도록 하기 위함이다.

### `status.md`

현재 Workflow 상태와 세션을 넘어 작업을 이어가기 위해 필요한 최소 정보를 기록한다.
...
작업의 성격과 복잡도에 따라 필요한 artifact만 사용할 수 있다.
...
Workflow가 활성화되면 최초 상태는 항상 Planning이다.
...
기본적으로 다음 전이에는 사용자의 명시적인 승인이 필요하다.
...
Planning은 실제 구현 기준이 될 Specification과 Design을 확정하는 단계이기 때문이다.

다음과 같은 의미의 사용자 응답을 승인으로 취급할 수 있다.
...
새로운 독립 작업을 Workflow로 수행하려면 프로젝트가 정의한 Workflow 진입 방법을 다시 사용한다.
