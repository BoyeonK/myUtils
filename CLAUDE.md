...
1. `progress.md` — 바로 착수 가능한 작업, 알려진 이슈
...
* **스레드 관리 주체를 요구하지 않는다.** 스레드가 필요한 컴포넌트는 자기가 만들어 자기가 소유하고, 소비자에게 스레드 등록·식별자·초기화 훅을 요구하지 않는다. 초기화 순서에 의존하는 전역도 두지 않는다. 상세: [`design/threading.md`](design/threading.md)
* **어느 컴파일러도 특별 취급하지 않는다.** 소스는 표준 C++17 + `concurrentqueue`뿐이고 플랫폼 헤더를 쓰지 않는다. 컴파일러별 분기는 늘리지 않는다 — 세 번째 자리를 만들어야 한다면 먼저 보고하고 결정을 받는다.
