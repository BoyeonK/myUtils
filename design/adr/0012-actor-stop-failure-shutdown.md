...
- Status: Accepted
- Date: 2026-09-15
- Follow-up: 2026-09-15 — 아래 "후속" 참고. 결정 #3에 drain 쪽 경계가 빠져 있었다.
...
## 후속 (2026-09-15) — consumption boundary

결정 #3("`Stop()`은 pending 메시지를 버린다")에 **drain 쪽 경계가 빠져 있었고, 1차
구현도 그대로 빠뜨렸다.** 결정이 바뀐 것이 아니라 결정을 실행하는 규칙이 없었다.
...
위 "왜 예외 시 Actor를 Stop하는가"에 이렇게 적혀 있다.

> `break`가 필수인 이유는 Stop이 state만 바꿀 뿐 worker는 여전히 drain 루프 안이기
> 때문이다. 탈출을 명시하지 않으면 다음 메시지를 계속 처리한다.

이 논증은 예외 경로를 설명하려고 쓴 것인데 **일반 `Stop` 경로에 글자 그대로 똑같이
적용된다.** 결정 #4에만 적용하고 #3에는 적용하지 않은 것이 누락이다. 결과적으로 같은
상황("이 Actor는 더 이상 메시지를 처리하면 안 된다")을 경로에 따라 다르게 처리하고
있었다.

증상은 UAF가 아니라 **비결정적 semantics**다. `Stop` 이후 몇 건이 더 처리될지가
타이밍에 달려 있어(0~31건) 호출자가 어느 쪽에도 기댈 수 없다. 즉시 중단도 graceful
drain도 아닌 중간 상태였다.

### 왜 `Handle()` 반환 직후가 아니라 락 안인가

`Stop`은 mailbox 락을 쥔 채 `STOPPING`을 쓴다(결정 #1). 그 락 안에서 읽으면 경계가
한 문장으로 닫힌다 — **"Stop이 mailbox 락을 통과한 시점 이후로는 어떤 메시지도 새로
pop되지 않는다."** `Send`의 acceptance boundary와 같은 락, 같은 형태의 진술이다.

`Handle()` 반환 직후에 검사하는 형태는 검사와 다음 pop 사이에 창이 남아 이 문장이
성립하지 않는다. 어차피 pop을 위해 잡는 락이므로 락 안에 두는 쪽이 추가 비용도 없다.

### 기각된 대안

| 대안 | 기각 사유 |
|---|---|
| 현재 배치는 끝까지 처리하고 다음 스케줄에서 멈춘다 | 처리 건수가 타이밍에 의존하는 문제가 그대로 남는다. budget이 커질수록 악화된다 |
| `Handle()` 반환 직후 검사 | 검사~pop 사이 창이 남아 경계를 문장으로 진술할 수 없다. 락 밖 atomic load가 메시지마다 추가된다 |
| `Stop`이 직접 mailbox를 비운다 | Finalize와 discard 책임이 둘로 나뉜다. L4(락 밖 파괴)를 `Stop` 경로에도 따로 지켜야 한다 |

### 검증

`SelfStopFromHandler`의 assertion을 "데드락하지 않음"에서 **"Stop 이후 pending이
실행되지 않음"**까지 확장했다. handler가 자기 자신에게 3건을 보낸 뒤 `Stop`하므로
메일박스에 메시지가 남아 있는 상황이 결정적으로 만들어진다 — 밖에서 연달아 `Send`하면
worker가 첫 건을 먼저 집어갈 수 있어 flaky해진다.

검사를 제거하면 이 테스트가 `handled == 4`로 실패하는 것을 확인했다.
